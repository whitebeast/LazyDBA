...
begin try
    if @pActivateDefragmentation = 1 begin;
        ...
        JOIN    [$(TargetDBName)].sys.tables tbl WITH(NOLOCK) ON dm.object_id = tbl.object_id
        JOIN    [$(TargetDBName)].sys.schemas s WITH(NOLOCK) ON tbl.schema_id = s.schema_id
        JOIN    [$(TargetDBName)].sys.indexes idx WITH(NOLOCK) ON dm.object_id = idx.object_id AND dm.index_id = idx.index_id
        left join (
        ...
        BEGIN
            ...
            insert into dbo.MaintenanceLog (ActionText, DateTime, ErrorMessage)
